...
public class LoginPanel : BasePanel {
	...
	public override void OnShow(params object[] args) {
		//寻找组件
		idInput = skin.transform.Find("Username").GetComponent<InputField>();
		pwInput = skin.transform.Find("Password").GetComponent<InputField>();
		...
		//NetManager.AddMsgListener("Msg", OnMsgLogin);
	...
	public void OnMsgLogin(ProtocolBase msgBase) {
        ...
        if (msgLogin1.result == 0) {
			...
			GameManager.Instance.id = idInput.text;

			//打开房间列表界面
			//  PanelManager.Open<RoomListPanel>();
			PanelManager.Open<LoadingPanel>();
			//关闭界面
			Close();
        }
        else {
